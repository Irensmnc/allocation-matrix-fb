  ...
  <v-card>
    ...
    >
      <template v-slot:items="props">
        ...
        <td class="text-xs-right">{{ props.item.info }}
          <v-textarea
            v-model="info"
            name="input-7-1"
            box
      ...
      submit() {
        ...
        console.log(project)
          if (project) {
            ...
            db.collection("users").doc(document.doc.id).update( usersUpdate);
          } else {
            db.collection('users')
              .doc(this.project.projectId).set({ projectId: this.project.projectId, email: this.email, info: this.info,})
              .then(() => {
                this.loading = false;
              });
          }
